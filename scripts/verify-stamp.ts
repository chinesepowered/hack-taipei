 ...
 *   pnpm verify-stamp <stamp> --onchain        additionally check the signer is the executor of the Agentic ID on 0G Galileo
...
async function main() {
  ...
  if (v.payload) {
    ...
    if (argv.includes("--onchain") && v.ok && v.signer) {
      const id = await checkIdentityOnChain({ agent_id_contract: p.agent_id_contract, agent_id_token: p.agent_id_token, signer: v.signer });
      console.log(`  on-chain id    ${id.checked ? (id.ok ? "MATCH" : "MISMATCH") : "SKIPPED"} - ${id.reason}`);
      if (id.explorer) console.log(`                 ${id.explorer}`);
    }
  }
